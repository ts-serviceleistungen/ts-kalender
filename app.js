...
async function load(){
  showLoadMessage('Kalender wird geladen …');
  ...
  if(reqRes.error){
    console.error('requests:',reqRes.error);
    showLoadMessage('Kundentermine konnten nicht geladen werden: '+reqRes.error.message);
  }

  const reqs=(reqRes.data||[])
    .filter(r=>r.requested_date && r.requested_time && String(r.status||'Neue Anfrage')!=='Abgelehnt')
    .map(r=>{
      const start=parseCalendarDate(r.requested_date,r.requested_time);
      if(!start){
        console.warn('Ungültiger Kundentermin übersprungen:',r);
        return null;
      }
  ...
  if(personalRes.error){
    console.error('personal_calendar_events:',personalRes.error);
    showLoadMessage('Persönliche Termine konnten nicht geladen werden: '+personalRes.error.message);
  }

  const personal=(personalRes.data||[]).map(x=>{
    const start=safeIso(x.start_time);
    const end=safeIso(x.end_time);

    if(!start || !end){
      console.warn('Ungültiger persönlicher Termin übersprungen:',x);
      return null;
    }
...
function parseCalendarDate(dateValue,timeValue){
  if(!dateValue || !timeValue) return null;

  let ds=String(dateValue).trim();
  let ts=String(timeValue).trim();

  // German date format, if present.
  const de=ds.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/);
  if(de) ds=`${de[3]}-${String(de[2]).padStart(2,'0')}-${String(de[1]).padStart(2,'0')}`;

  // Keep only the clock part from values like "14:30:00+00".
  const tm=ts.match(/(\d{1,2}):(\d{2})(?::(\d{2}))?/);
  if(!tm) return null;

  const iso=`${ds}T${String(tm[1]).padStart(2,'0')}:${tm[2]}:${tm[3]||'00'}`;
...
function showDataStatus(requestCount, personalCount, requestError, personalError){
  let el=$('calendarDataStatus');
  if(!el){
    el=document.createElement('p');
    el.id='calendarDataStatus';
    el.className='muted';
  ...
  const parts=[
    `Kundentermine: ${requestCount}`,
    `eigene Termine/Schichten: ${personalCount}`
  ];

  if(requestError) parts.push('Kundentermine-Fehler: '+requestError);
  if(personalError) parts.push('Eigene Termine-Fehler: '+personalError);
...
function eventsOn(d){
  const target=berlinDateInput(d);
  return events
    .filter(e=>{
      // 24h-Schichten gehören im Kalender zum Starttag.
      if(e.event_type==='shift') return berlinDateInput(e.start)===target;

      // Alle anderen Termine werden nach tatsächlicher Überschneidung mit
      // dem Kalendertag angezeigt.
      const dayStart=new Date(`${target}T00:00:00`);
      const next=new Date(dayStart);
      next.setDate(next.getDate()+1);
      return new Date(e.start)<next && new Date(e.end)>dayStart;
    })
...
$('prevBtn').onclick=()=>moveCalendar(-1);
$('nextBtn').onclick=()=>moveCalendar(1);
...
// Alte Service Worker entfernen, damit keine alte Kalender-Version mehr geladen wird.
if('serviceWorker' in navigator){
  ...
  if(window.caches){
    caches.keys().then(keys=>{
      keys.filter(k=>k.startsWith('ts-kalender-')).forEach(k=>caches.delete(k));
    }).catch(()=>{});
  }
}
