...
const BAYERN_SCHOOL_HOLIDAYS=[
  ...
  ['Allerheiligenferien','2026-11-02','2026-11-06'],
  ...
  ['Allerheiligenferien','2027-11-02','2027-11-05'],
  ...
  ['Allerheiligenferien','2028-10-30','2028-11-03'],
  ...
  ['Allerheiligenferien','2029-10-29','2029-11-02'],
...
function holidayForDate(d){
  ...
  return BAYERN_SCHOOL_HOLIDAYS.find(h=>key>=h[1] && key<=h[2]) || null;
}
...
function parseCalendarDate(dateValue,timeValue){
  ...
  let normalizedTime=ts;
  if(lower.includes('vormittag')) normalizedTime='08:00';
  else if(lower.includes('nachmittag')) normalizedTime='13:00';
  else if(lower.includes('abend')) normalizedTime='17:00';

  const tm=normalizedTime.match(/(\d{1,2}):(\d{2})(?::(\d{2}))?/);
  if(!tm) return null;
...
async function load(){
  ...
  const personal=(personalRes.data||[]).map(x=>{
    ...
    const start=safeDate(x.start_time);
    const end=safeDate(x.end_time);
    if(!start || !end) {
...
function renderMonth(){
  ...
  monthCells().forEach(d=>{
    ...
    h+=`<div class="day ${muted?'muted':''}${holiday?' school-holiday':''}"><div class="day-num">${d.getDate()}</div>${holiday?`<div class="holiday-label" title="Bayerische Schulferien">${esc(holiday[0])}</div>`:''}${eventsOn(d).map(eventHtml).join('')}</div>`;
  });
