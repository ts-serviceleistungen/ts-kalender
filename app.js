...
async function load(){
  ...
  // Der bestehende Admin-Kalender basiert auf public.requests.
  // request_booking_blocks ist aktuell leer. Deshalb lesen wir die
  // Kundentermine direkt aus requests – wie im bestehenden Admin-Kalender.
  const reqRes=await db
    .from('requests')
    .select('id,service_type,status,requested_date,requested_time,care_options')
    .gte('requested_date',berlinDateInput(from))
    .lte('requested_date',berlinDateInput(to));

  if(reqRes.error){
    console.error('requests:',reqRes.error);
    events=[];
    render();
    showLoadMessage('Die bestehenden Kundentermine konnten nicht geladen werden: '+reqRes.error.message);
    return;
  }

  const reqs=(reqRes.data||[])
    .map(r=>{
      if(!r.requested_date || !r.requested_time) return null;
  ...
  // Eigene Termine und 24h-Schichten.
